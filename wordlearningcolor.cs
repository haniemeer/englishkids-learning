...
{
    public partial class wordlearningcolor : Form
    {
        Imycourserepository course;

        public int parentId = 0;
        public int flashcartId = 0;
        public wordlearningcolor()
        {
            InitializeComponent();
        ...
        private void pervi_color_Click(object sender, EventArgs e)
        {
            System.Media.SoundPlayer player;
            player = new System.Media.SoundPlayer(@"audio\color");
            player.Stop();
            clickBtn(flashcartId - 1);
        }

        private void next_color_Click(object sender, EventArgs e)
        {
            clickBtn(flashcartId + 1);
        }

        private void wordlearningcolor_Load(object sender, EventArgs e)
        {
            pervi_color.Enabled = false;
            parentId = Form2.parent_id;
            var courses = course.course(parentId, 47);
            ...
            col_flpic.Image = Image.FromFile("image\\color\\" + img2);
            System.Media.SoundPlayer player;
            player = new System.Media.SoundPlayer("audio\\color\\" + audio);
            player.Play();
        }
