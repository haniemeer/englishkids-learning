...
{
    public partial class wordlearningnumber : Form
    {
        Imycourserepository course;

        public int parentId = 0;
        public int flashcartId = 0;
        public wordlearningnumber()
        {
            InitializeComponent();
            course = new Courserepository();
        }

        private void back_pi_Click(object sender, EventArgs e)
        {
        ...
        private void pervi_num_Click(object sender, EventArgs e)
        {
        ...
        private void next_num_Click(object sender, EventArgs e)
        {
            clickBtn(flashcartId + 1);
        }

        private void wordlearningnumber_Load(object sender, EventArgs e)
        {
            pervi_num.Enabled = false;
            parentId = Form2.parent_id;
            var courses = course.course(parentId, 27);
            ...
            num_flpic.Image = Image.FromFile("image\\number\\" + img2);
            System.Media.SoundPlayer player;
            player = new System.Media.SoundPlayer("audio\\number\\" + audio);
            player.Play();
        }
        public void clickBtn(int flashId)
        {
            ...
            {
             pervi_num.Enabled = true;
            }

            if (flashId == 46)
            {
                next_num.Enabled = false;
            }
            else
            {
             next_num.Enabled = true;
            }
